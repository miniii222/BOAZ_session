...
####데이터 불러오기####
setwd("C:\\Users\\wjssm\\Desktop\\boaz\\1.전처리\\R")   ##지정된 곳에 파일이 있어야함
...
############################################################################################
titan%>%filter(sex=="female", age>=20, age<30)%>%select(pclass,survived,sex,age)
...
####################################################################################################################
titan%>%mutate(fare_norm=(fare-mean(fare, na.rm=T))/sd(fare,na.rm=T))%>%select(fare_norm)%>%head()
...
titan%>%filter(pclass==1, age>=30, age<40, sex=="female")%>%group_by(survived)%>%summarise(count=n())
