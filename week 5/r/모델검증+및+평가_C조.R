...
setwd("C:/Users/wjssm/Desktop/boaz/week 5/r")
...
logis_model <- glm(반품여부~.-ID, data=data.train, family='binomial')
# confusion matrix로 확인

rf_pred<-predict(rf_model, data.test, type="response")
confusionMatrix(rf_pred, data.test$반품여부)

logis_pred <- predict(logis_model, data.test, type="response")
logis_pred <- ifelse(logis_pred>=0.5,1,0); logis_pred<-factor(logis_pred)
confusionMatrix(logis_pred, data.test$반품여부)
...
data.test$logis_prob <- predict(logis_model, data.test,'response')#probability
logis_pred2 <- prediction(data.test$logis_prob, data.test$반품여부)
plot(performance(logis_pred2, 'tpr', 'fpr'))
...
performance(logis_pred2, 'auc')@y.values[[1]]
...
data.test$xgb_prob <- predict(xgb.model, data.test, type='prob')#probability
xgb_pred2 <- prediction(data.test$xgb_prob[,2], data.test$color)
plot(performance(xgb_pred2, 'tpr', 'fpr'))
performance(xgb_pred2, 'auc')@y.values[[1]]
